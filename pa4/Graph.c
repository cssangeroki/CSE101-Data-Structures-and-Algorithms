...
void visit(Graph G, List S, int x, int *time){
    ...
    for(moveFront(G->adjacent[x]);(index(G->adjacent[x])) != 1; moveNext(G->adjacent[x])){
...
Graph transpose(Graph G){
    ...
    for(int i = 0; i < getOrder(G); i++){
        ...
        moveFront(G->adjacent[i+1]);
        while(index(G->adjacent[i+1]) >= 0){

            addArc(G2, get(G->adjacent[i+1]), i+1);
            moveNext(G->adjacent[i+1]);
